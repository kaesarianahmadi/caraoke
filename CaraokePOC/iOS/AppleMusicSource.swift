...
/// Ported from DriveVerse `Core/NowPlaying/AppleMusicSource.swift` (MIT © 2026
/// Praveet Gupta, see THIRD_PARTY_NOTICES.md).
///
/// Key architectural fact this module locks in: Apple Music playback is read
/// through the public MediaPlayer framework (`MPMusicPlayerController
/// .systemMusicPlayer`) — no MusicKit, no developer token, no backend. This
/// supersedes Caraoke PRD decision D5 ("Apple Music via MusicKit").
...
final class AppleMusicSource: NowPlayingSource {
    ...
    private func beginObserving() {
        ...
        // flaky while backgrounded, so track switches and pauses must also be
        // caught by polling, not notifications alone.
